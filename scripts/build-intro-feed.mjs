...
const coverResponse=await fetch(`https://image.tmdb.org/t/p/w1280${items[0].backdrop_path}`);
if(!coverResponse.ok)throw new Error(`Imagem da intro ${coverResponse.status}`);
await fs.writeFile('intro-cover.jpg',Buffer.from(await coverResponse.arrayBuffer()));
